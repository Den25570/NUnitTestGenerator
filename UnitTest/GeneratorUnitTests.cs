...
namespace UnitTest
{
    public class GeneratorUnitTests
    {
        ...
        public void TestMethodNames()
        {
            ...
            Assert.AreEqual(1, methodsOne.Where((method) => method.Identifier.ToString() == "TestShitTest").Count());

            IEnumerable<MethodDeclarationSyntax> methodsTwo = classTwoRoot.DescendantNodes().OfType<MethodDeclarationSyntax>();

            Assert.AreEqual(1, methodsTwo.Where((method) => method.Identifier.ToString() == "TestShit1Test").Count());
            Assert.AreEqual(1, methodsTwo.Where((method) => method.Identifier.ToString() == "TestShit2Test").Count());
        ...
        public void TestSetupMethod()
        {
            ...
            Assert.AreEqual(1, variables.Where((variable) => variable.GetText().ToString().Contains("disposable = new Mock<IDisposable>()")).Count());
            Assert.AreEqual(1, variables.Where((variable) => variable.GetText().ToString().Contains("classForTest1 = new ClassForTest1(a, disposable)")).Count());
        }
        ...
        {
            IEnumerable<MethodDeclarationSyntax> methodsOne = classOneRoot.DescendantNodes().OfType<MethodDeclarationSyntax>();
            var method = methodsOne.Where((method) => method.Identifier.ToString() == "TestShitTest").First();
            var variables = method.DescendantNodes().OfType<VariableDeclarationSyntax>();
            var methodCalls = method.DescendantNodes().OfType<InvocationExpressionSyntax>();

            Assert.AreEqual(1, variables.Where((variable) => variable.GetText().ToString().Contains("int shit = 0")).Count());
            Assert.AreEqual(1, variables.Where((variable) => variable.GetText().ToString().Contains("int actual = classForTest1.TestShit(shit)")).Count());
